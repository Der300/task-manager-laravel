...
class TaskService
{
     ...
     */
    public function moveTaskFilesToTrash(Task $task): void
    {
        $files = DB::table('files')->where('task_id', $task->id)->get();

        foreach ($files as $file) {
            $storedName = $file->stored_name;
            $originalPath = public_path($this->fileFolder . $storedName);
            $trashPath = public_path($this->fileFolderTrash . $storedName);
            ...
            if (file_exists($originalPath)) {
                ...
                DB::table('files')
                    ->where('id', $file->id)
                    ->update(['path' => $this->fileFolderTrash . $storedName]);
            }
        }
    }
     ...
     * @param Task $task tên task
     */
    public function restoreTaskFilesFromTrash(Task $task): void
    {
        ...
        foreach ($files as $file) {
            ...
            if (file_exists($trashPath)) {
                ...
                DB::table('files')
                    ->where('id', $file->id)
                    ->update(['path' => $this->fileFolder . $storedName]);
            }
        }
    }
     ...
     * @param Task $task tên task
     */
    public function deleteTaskFilesPermanently(Task $task): void
    {
        $files = DB::table('files')
            ->where('task_id', $task->id)
            ->get();
